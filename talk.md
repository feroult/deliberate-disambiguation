...
> **Prompt de imagem:** Minimalist schematic diagram on a near-black background (#0d0d14), 16:9 aspect ratio, flat design, no shadows, no 3D effects. Two nodes sit symmetrically at the top and bottom center of the frame: the top node is a clean rounded rectangle labeled "D1" in bold white geometric sans-serif with "Ignorância do domínio" in smaller regular white type below; the bottom node is a clean rounded rectangle labeled "D2" in bold white with "Ambiguidade da linguagem" below. Both nodes are fully sharp, white text, white border — no fog. Between them, three concentric oval loops share the same two nodes as anchor points, each loop progressively smaller and tighter than the last: the outermost oval is the widest, rendered in white at 35% opacity, thin stroke; the middle oval is tighter, white at 60% opacity, slightly thicker stroke; the innermost oval is the tightest, white at 90% opacity, thickest stroke — closest to a luminous central point between the two nodes. This progression of shrinking ovals is the convergence: each iteration of the loop reduces the ambiguity space. Each oval has two directional arrowheads — one on each arc — showing the clockwise flow of the cycle. The outermost oval carries the arc labels: the right arc labeled "resolver D1 abre espaço para D2" in small white type, the left arc labeled "tentar D2 revela D1" in small warm amber type (#e8a020). At the center of the nested ovals, where the loops converge, a small bright dot or tight point in warm amber-white — the implied destination of the process, unlabeled. The composition is perfectly symmetric on the vertical axis. Typography: geometric sans-serif throughout, white. No fog, no textures, no gradients. The convergence is structural — the geometry does the work. Aspect ratio 16:9.
