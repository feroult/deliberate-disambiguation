...
> **Prompt de imagem:** Clean technical infographic schematic on a near-black background (#0d0d14), 16:9 aspect ratio, flat design with no shadows or 3D effects. Three vertical zones are arranged side by side across the central 80% of the frame. The left zone is labeled "Ignorância 2ª ordem" in bold white sans-serif at top and "não sei o que não sei" in smaller type below — but the entire zone is rendered under a heavy blue-grey fog overlay at 60% opacity, edges undefined and bleeding softly into the background, the label text itself slightly blurred as if seen through haze, the dashed border barely visible. The center zone is labeled "D1" large and bold at top, "Ignorância 1ª ordem" and "sei que não sei X" below — clearer than the first, a faint residual haze at 25% opacity, label text fully white and legible, a thin 1px white border at 50% opacity, suggesting a problem that has been named but is not yet resolved. The right zone is labeled "D2" large and bold at top, "Ambiguidade da linguagem" and "formalização" below — no fog at all, maximum contrast, fully sharp, 1px white border at 90% opacity, crisp and precise. A single horizontal white arrow at mid-height connects all three zones left to right, labeled beneath it "scout → exploração → formalização", clean arrowhead pointing right. A second arrow — curved, dashed, warm amber (#e8a020), 4px dash 4px gap — arcs from the right edge of the D2 zone upward and loops back to the center of the D1 zone, with an arrowhead pointing left and a label along the arc reading "tentar resolver D2 revela D1" in amber type. The amber return arrow is the visual center of attention: it is the loop. Typography throughout is geometric sans-serif, white except for the amber arc label. The dominant visual narrative is the left-to-right gradient from fog to clarity — the eye travels from indistinction to precision, then the amber arc brings it back. Aspect ratio 16:9.
