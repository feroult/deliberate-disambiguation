...
O colapso do custo de formalização não resolve a ambiguidade do mundo: ele a amplifica. Cada sistema construído adiciona comportamento ao ambiente em que outros sistemas precisam operar. O território fica mais complexo à medida que se torna mais barato construir nele. A primeira dimensão cresce: há mais domínio desconhecido emergindo de composições que ninguém previu. A segunda se multiplica: cada camada de delegação entre intenção e execução carrega imprecisão acumulada. O humano que sabe desambiguar deliberadamente não se torna menos necessário nesse contexto. Torna-se o fator limitante.
